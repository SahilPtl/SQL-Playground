...
test("invalid optional credentials do not prevent local signup, SQL, Coach or Interview", async () => {
  ...
  assert.equal(
    (
      await agent
        ...
        .send({ sql: "SELECT * FROM employees" })
    ).body.success,
    true,
  );
  ...
  try {
    assert.equal(
      (
        await agent
          .post("/api/coach")
          .send({ action: "explain", text: "SELECT * FROM employees" })
      ).body.provider,
      "Local Coach",
    );
  } finally {
  ...
  const oauth = await agent.get("/api/auth/google");
  assert.equal(oauth.status, 302);
  assert.match(oauth.headers.location, /accounts.google.com/);
  const rejected = await agent.get(
  ...
  assert.equal(
    ...
    "Invalid config learner",
  );
  const start = (
    await agent.post("/api/interview/challenges/salary-filter/start").send({})
  ).body;
  const result = (
    await agent
      .post("/api/interview/challenges/salary-filter/submit")
      .send({ attemptId: start.attempt.id, sql: challenges[0].solution })
  ).body;

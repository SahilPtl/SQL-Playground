...
export class SQLiteSessionStore extends session.Store {
  get(sid, done) {
    try {
      const row = appDb
        ...
        .get(sid, Date.now());
      done(null, row ? JSON.parse(row.data) : null);
    } catch (e) {
      done(e);
    }
  }
  set(sid, data, done = () => {}) {
    try {
      ...
      appDb
        ...
        .run(sid, JSON.stringify(data), Date.now() + 86400000);
      done();
    } catch (e) {
      done(e);
    }
  ...
  destroy(sid, done = () => {}) {
    try {
      appDb.prepare("DELETE FROM sessions WHERE sid=?").run(sid);
      done();
    } catch (e) {
      done(e);
    }
...
export function sessionSecret() {
  ...
  if (!existsSync(file))
    writeFileSync(file, randomBytes(48).toString("hex"), { mode: 0o600 });

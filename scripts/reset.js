...
if (!process.argv.includes("--confirm")) {
  console.log(
    "Reset backs up ALL local accounts, sessions and workspaces. Stop the demo first, then run npm run reset -- --confirm. For one workspace use the app’s Reset workspace button.",
  );
  process.exit(0);
}
for (const port of [5000, 5173, 15000, 15173]) {
  try {
    ...
    console.error("Stop the demo with npm run stop before resetting.");
    process.exit(1);
  }
}
const root = path.resolve("server"),
  source = path.resolve(root, "data");

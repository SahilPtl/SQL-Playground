...
- OAuth, OpenAI and session secrets: server environment/private local storage. No `VITE_*` secrets. Workers receive only PATH and SystemRoot environment variables.
...
| Provider failures                  | 6-second timeout, 500 output tokens, generic errors, deterministic fallback                                         | Mock 401/429/500/network cases and real invalid-key request      |
